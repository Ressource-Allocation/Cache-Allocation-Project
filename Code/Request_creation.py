...
def init():
    ...
    global liste_nb_video #Number of videos provided by each CP
    ...
    list_proba=[0.7, 0.25, 0.05] #test to try and find convergence
    liste_nb_video=gn.liste_100_videos(k) #100 videos by CP
...
def request_creation(): #k
    ...
    choix_CP = rd.random()
    while(1==1):
        somme=somme+list_proba[i]
        if(choix_CP <= somme):
            break
        else:
            i +=1
    CP=i
    distribution = zipf_distribution(list_alpha[i], liste_nb_video[i], conss_zipf[i]) #conss_zipf permet de soulages les calculs des constantes de normalisation de zipf
...
def decide_naive_alloc(): # cache_capacity, k
    ...
    nb_video_total=af.list_sum(liste_nb_video)
    for i in range(k):
...
def decide_opt_alloc(): # cache_capacity, k
    ...
    allocation=[0]*k
    pointeurs_max=[0]*k
    if cache_capacity > af.list_sum(liste_nb_video):
        return 'Erreur: Le cache est trop grand par rapport au nombre total de videos'
    for i in range(k):
        distribution[i]=zipf_distribution(list_alpha[i], liste_nb_video[i], conss_zipf[i])
        popularite[i] = [piyt * list_proba[i] for piyt in distribution[i]] #liste que l'on va comparer
...
def sarsa_pour_3(request_rate, nb_intervalle, taille_intervalle): #intervalle, request_rate, gamma, epsilon, cache_capacity, alpha
    ...
    while (k<nb_intervalle): #tous 100 intervalles
        liste_cout_moyen.append((af.list_sum(liste_cout[k : k+100]) / 100.0))
        k += 100
...
def tests_de_gamma(request_rate, nb_intervalle, taille_intervalle):
    ...
    for k in liste_gamma:
        ...
        while (i<len(cout_du_sarsa)): #tous 10 intervalles
            liste_cout_moyen.append((af.list_sum(cout_du_sarsa[i : i+10]) / 10.0))
            i += 10
