...
def sarsa_nSP(request_rate, nb_interval, interval_size, gama, epsi, alfa,delta): #intervalle, request_rate, gamma, epsilon, cache_capacity, alpha
    ...
    gama: float
        Usual gamma parameter of SARSA's algorithm
    epsi: float
        Usual epsilon parameter of SARSA's algorithm
    alfa: float
        Usual alpha parameter of SARSA's algorithm
    delta: int
        Number of slots by which we modify the storage allocation 
    ...
    L_first_cost: list of float
        list containing the evolution over interval of the cost of the initial allocation (independant of SARSA's algorithm)
    L_best_cost: list of float
        list containing the evolution over interval of the cost of the optimal allocation (independant of SARSA's algorithm)    
    """   
    init()
    ...
    count_alea=0 #number of random taken actions
    count_best=0 #number of best taken actions
    ###### ACTION %%%%%
    for j in range(nb_interval):
        ...
        if allocation_ok:
            ...
            total_cost =(nominal_cost+allocation[action_plus]-old_allocation[action_plus])/request_nb
            nominal_cost=nominal_cost/request_nb
            first_cost/=request_nb
            best_cost=best_cost/request_nb                
            new_gain =1-nominal_cost  # Reward
            #if DEBUG:
            ...
            if action_plus==action_minus:
                for act in range(nSP):
                    Q[act*nSP][state_index]+=alfa*(new_gain+gama*best_score1-Q[act][state_index]) #SARSA max
                    #Q[act*nSP][state_index]+= alfa*(new_gain + gama*Q[act][state_index_prime] - Q[act][state_index]) #classic SARSA
                V[action][state_index]+=1
            else:
               Q[action][state_index]+=alfa*(new_gain+gama*best_score1-Q[action][state_index])
                    #Q[action*nSP][state_index]+= alfa*(new_gain + gama*Q[action][state_index_prime] - Q[action][state_index])
            #if DEBUG:
